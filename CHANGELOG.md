...
### Changed
- **`Snapshot` gained a cell for Track E.** `claude_statusline` carries the live `StatuslineFilePayload` envelope (`captured_at` + `payload`, i.e. `rate_limits.{five_hour,seven_day}` + `cost.total_cost_usd` — Claude Code's session estimate, a third explicitly-labeled cost tier per Track C's honesty discipline).
- Replaced the EWMA predictor with a measured **pace view** (`Snapshot.pace`, `--json .pace`): per-window used % vs elapsed % + ratio, computed by the pure `window::pace`. The `predictor` crate is retired. The compact view is reshaped to the measured-only matrix (R1): the JSONL list-price estimate moved out of the Anthropic API-$ cell to a separate "Subscription leverage" line, with a new "Pace:" line.
...
### Fixed
- **JSONL discovery now unions every project root.** A machine with both `~/.claude/projects` and `~/.config/claude/projects` (Claude Code's path migration, or a dual install) previously had only the *first* existing root read — events and the API-$ estimate under the other root were silently undercounted. New `claude_parser::find_all_claude_projects_dirs()` returns all existing roots (deduped by canonical path so a symlinked root isn't walked twice); the CLI loader, the watcher JSONL notify task (now watches each root), and the safety poll all walk + concatenate across them. The existing `(message_id, request_id)` dedup collapses any session that appears under more than one root.
