...
│   ├── claude_cost/            pure JSONL→estimated-$ synthesis. `compute_cost(&[UsageEvent], &PriceTable) -> Cost` is infallible (unknown models → `skipped_models`, empty-model → `unparsed_event_count`); i64 micro-USD with i128 intermediates, saturates at `i64::MAX`. `build.rs` codegens `PRICE_TABLE_COMMIT`/`_DATE` from the vendored LiteLLM Anthropic-subset JSON (MIT; `LICENSE-LITELLM` committed alongside). Pure like `window` — no I/O, no logging above debug. 37 unit tests + deduped smoke. Output is an **estimate / subscription-leverage** figure, never presented as real spend.
...
│   ├── codex_local/            reads `~/.codex/sessions/{YYYY}/{MM}/{DD}/rollout-*.jsonl`; extracts the latest `rate_limits.primary` from the newest session's last `token_count` event. Returns a single `CodexQuotaSnapshot` (NOT a stream — Codex doesn't stream-duplicate, so no dedup module; no IncrementalParser in v0.1). `thiserror` enum: `FileMissing`/`IoError`/`SchemaDrift`. Honors `CODEX_CONFIG_DIR`. **Only crate that reads `~/.codex/`** (boundary #11). 15 unit tests + smoke.
...
- The backend data layer is shipped (10 crates: claude_parser, claude_cost, anthropic_oauth, openai_client, codex_local, settings, keychain, window, state_coordinator, balanze_cli). The Tauri frontend is still scaffold only (greet form + tray menu). As remaining crates land (`predictor`, `watcher`) this AGENTS.md grows with them — each adds an entry to the Repo Map and may add a row to the Validation Matrix.
...
| `crates/claude_cost/**` | All Rust gates + parser/compute unit tests (per-model aggregation, unknown-model → `skipped_models`, empty-model → `unparsed_event_count`, nano→micro half-up rounding, i128 saturation at `i64::MAX`, sort determinism, deterministic output) + bundled-price-table load + `build.rs` provenance codegen + smoke run against real `~/.claude/projects/` (deduped) |
| `crates/codex_local/**` | All Rust gates + parser unit tests (latest `token_count` extraction, `rate_limits.primary` mapping, missing/empty/malformed → typed `ParseError`, `SchemaDrift` when ≥1 `token_count` seen but 0 parsed) + walker tests (3-level `YYYY/MM/DD` descent, newest-session pick) + tempfile cycles + smoke run against real `~/.codex/sessions/` |
...
Current state: **186 tests (187 on Linux — the Unix-gated walker permission test) across 10 shipped crates** (9 backend + balanze_cli glue) plus a 4-test end-to-end integration suite; 2 crates still planned (predictor, watcher, plus the Tauri smoke). One `#[ignore]`'d keychain smoke. As crates land, the table fills in.

| Subject | Location | What's there |
|---|---|---|
| Claude JSONL parsing | `crates/claude_parser/src/{parser,walker,dedup,incremental}.rs` (inline `#[cfg(test)]`) + `examples/smoke.rs` | 49 unit tests: parser (happy/zero-cache/extra-fields/blank/malformed/missing-ts/blank-skips/error-line-numbers/partial-final-line/message_id-request_id extraction/missing-ids) + walker (recursive find/missing-root/empty-dir/mtime-sort/+1 Unix-only permission-denied) + `find_claude_projects_dir` (XDG present/absent/empty, dot-claude existence, dot-config-claude fallback, XDG-over-dot-claude preference) + dedup (8: empty/no-dups/first-wins/order-preserved/None msg_id passthrough/None req_id passthrough/same-msg-different-req distinct/mixed keyed-and-unkeyed) + IncrementalParser (18: 6 pure-decision + 5 prefix-length + 7 tempfile cycles for first-read / unchanged / append / partial-then-complete / truncation / invalidate / missing). Smoke example runs against real `~/.claude/projects/` (`cargo run --release -p claude_parser --example claude_parser_smoke`). |
| Claude OAuth usage | `crates/anthropic_oauth/src/{client,credentials}.rs` (inline `#[cfg(test)]`) + `tests/wiremock_tests.rs` + `examples/smoke.rs` | 17 unit tests covering parser response-shape happy/null-cadences/unknown-key-titlecase/sort-order/malformed-cadence/non-object-root + credentials happy/minimal/missing/malformed/missing-key + 3 error-body redaction tests (sk-ant token masked, short non-key passthrough, overlong-body truncation). 5 wiremock integration tests for HTTP layer: happy/401/500/invalid-json/missing-org-header. Smoke example runs against the real endpoint. Refresh-token flow is a later-phase TODO (v0.1.1). |
...
| Estimated Claude cost | `crates/claude_cost/src/{compute,prices,errors}.rs` (inline `#[cfg(test)]`) + `examples/cost_smoke.rs` | 37 unit tests: compute (per-model aggregation, unknown-model → `skipped_models`, empty-model → `unparsed_event_count`, nano→micro half-up rounding, i128 saturation at `i64::MAX`, sort determinism, deterministic output) + prices (bundled-table load, validate_price rejects non-finite/negative/≥$1-per-token) + `build.rs` provenance constants. Smoke runs deduped against real `~/.claude/projects/` (`cargo run --release -p claude_cost --example cost_smoke`); its total reconciles with `balanze-cli`. |
...
| OpenAI billing client | `crates/openai_client/src/client.rs` (inline `#[cfg(test)]`) + `tests/wiremock_tests.rs` | 12 parser unit tests (multi-bucket aggregation, line-item sort, null-line-item-as-unknown, missing-amount-skipped, truncation flag, amount-as-string-OR-number deserializer, etc.) + 6 redaction unit tests + 5 wiremock integration tests (happy with admin Bearer + query params, 401 → AuthInvalid, 403 → InsufficientScope with admin-key hint, 500, invalid-JSON). |
| Codex quota | `crates/codex_local/src/{parser,walker}.rs` (inline `#[cfg(test)]`) + `examples/codex_local_smoke.rs` | 15 unit tests: parser (latest `token_count` extraction, `rate_limits.primary` mapping, missing/empty/malformed → typed `ParseError`, `SchemaDrift` when ≥1 `token_count` seen but 0 parsed) + walker (3-level `YYYY/MM/DD` descent, newest-session pick, tempfile cycles). Smoke runs against real `~/.codex/sessions/`. |
| StateCoordinator actor | `crates/state_coordinator/src/{snapshot,coordinator}.rs` (inline `#[cfg(test)]`) | 15 unit tests: snapshot (8: merge_partial per source clears matching _error incl. AnthropicApiCost + CodexQuota, record_error preserves data, record_error routes correctly, cross-source error isolation — a merge clears only its own _error) + coordinator (7: Update happy → merge + on_snapshot, Update Err → record_error + on_degraded, Query returns current Snapshot, Refresh re-notifies sink, SettingsChanged is non-panicking, mpsc-burst-no-drops with tight 4-slot channel + 32 messages, handle clone shares same coordinator). |
| File watcher | `crates/watcher/tests/` | (planned) `tokio::time::pause()` timing tests |
| Settings serde | `crates/settings/src/lib.rs` (inline `#[cfg(test)]`) | 10 unit tests: load missing → defaults, load corrupt → Malformed, atomic save uses tmp + rename, schema version handling, explicit `version==0` migrated to current (distinct from the serde-defaulted omitted-version case), unknown extra-fields tolerated, minimal version-only file loads. |
...
| 4-quadrant E2E | `crates/balanze_cli/tests/integration_4quadrant.rs` + `tests/fixtures/` | 4 tests exercising the real `claude_parser → claude_cost → Snapshot` and `codex_local → Snapshot` composition against committed fixtures (Claude JSONL with a deliberate duplicate line so `dedup_events` is genuinely exercised; Codex rollout with a known `used_percent`). Fixed `now` so window math is deterministic, not wall-clock-flaky. Catches wiring regressions unit tests miss. |
...
The v0.1 backend data layer is shipped (10 crates, 186 tests + a 4-test E2E suite); the Tauri tray UI is still scaffold. This section captures the footguns the design surfaced plus anything observed in development.
