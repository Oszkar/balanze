...
- HTTP retry/backoff: lives in `anthropic_oauth` + `openai_client` via the `backoff` crate (exponential, policy-parameterized — see `crates/backoff/` in the Repo Map). Idempotent GETs (`fetch_usage`, the OpenAI costs endpoint) retry on 429 + 5xx + transport errors; the token-ROTATING refresh-token POST (`refresh_access_token`) retries **429-only** — a 5xx/timeout retry could replay a consumed refresh token and strand the user. The one-shot CLI passes `BackoffPolicy::fail_fast()` (0 retries — never block an interactive invocation on provider backoff); the future v0.2 watcher passes `BackoffPolicy::standard()` (exponential, start 30s, cap 10 min — per the cadence numbers above).
