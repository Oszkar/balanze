...
- **Single reader.** `anthropic_oauth` is the only crate that reads Claude Code's OAuth credential. Every credential representation belongs to Claude Code and is strictly **read-only** to Balanze: file sources are re-read each poll to observe Claude Code's atomic replacements, while the macOS login Keychain source is cached while valid to avoid repeated access prompts. Balanze never exchanges the rotating refresh token, writes, mirrors, persists, or backs up credential contents. An expired or rejected credential surfaces `OAuthError::CredentialExpiredReadOnly` ("re-run `claude login`"). The Keychain read shells out to `/usr/bin/security` (no new dep, keeps `keychain` the sole keyring-importing crate). A future explicit file-refresh opt-in is tracked in [#186](https://github.com/Oszkar/balanze/issues/186); it requires separate settings-schema and secret-boundary review.
