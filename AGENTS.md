...
4. **`watcher` owns `notify` + the debounce + the 60s safety poll.** No other crate imports `notify`. `Watcher::spawn(handle, settings) -> Vec<JoinHandle<Result<(), WatcherError>>>` is the entry-point: one `JoinHandle` per long-running task (JSONL notify; statusline notify, OAuth poll, OpenAI poll, 60s safety — these latter four land with v0.2 Track E 5b/6). The caller (`balanze-cli --watch` supervisor or the v0.3 Tauri host) runs them under `tokio::select!` and decides restart/abort policy. The earlier "exposes an `mpsc::Receiver<WatchEvent>` and that's it" contract was a pre-implementation sketch; the shipped contract is handle-based because the watcher feeds the coordinator directly via `StateCoordinatorHandle::send(StateMsg::Update(...))` rather than emitting raw events for a downstream consumer to interpret.
