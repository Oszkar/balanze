...
| Currency | `i64` micro-USD (1e-6 USD) internally. Never use `f64` for sums or threshold comparisons - and no longer for display either: the `money` crate owns the one display policy (integer rounding to whole cents, half away from zero), because `{:.2}` over a float disagreed with the frontend's `Intl.NumberFormat` at a half cent. Every Rust surface calls it; the frontend mirrors the same rounding, and the `money` vectors in `tests/fixtures/presentation-policy.json` hold the two languages together |
