...
- HTTP retry/backoff: implemented in `anthropic_oauth` + `openai_client` via the `backoff` crate. Idempotent GETs retry on 429 + 5xx + transport. CLI passes `BackoffPolicy::fail_fast()` (0 retries); the watcher passes `BackoffPolicy::standard()` (30s start, 10-min cap). Balanze never calls Anthropic's token-refresh endpoint.
- Statusline self-compose path: when `balanze-cli statusline` runs without a fresh `snapshot.json`, it fetches OpenAI costs directly through a 300s on-disk cache and a short 3s client timeout - but only when a configured statusline line actually contains `{openai_cost}` (`want_openai`, threaded into `self_compose(..., want_openai)`); otherwise the whole OpenAI leg is skipped, no cache read, no refresh lease, no HTTP. The cache uses a key fingerprint, a matching 300s failure cooldown, and a dependency-free `create_new` refresh lease with an under-lease cache recheck, bounded waiting when no value exists, 10s abandoned-lease recovery, atomic publication, and immediate stale-data service while another process refreshes. The OpenAI key is resolved once per invocation and the same owned value drives both its fingerprint and request. This honors the 5-minute OpenAI gate machine-wide across concurrent turns and processes. It never calls the Anthropic OAuth usage endpoint (see `docs/ARCHITECTURE.md` "Self-compose fallback cache").
