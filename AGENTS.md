...
- OpenAI Costs endpoint: every caller (`status`, `export`, watcher, setup/doctor/Tauri validation, and statusline self-compose) goes through `openai_client`'s durable gate. The request identity is the exact API key plus normalized API base URL plus operation tag; disk stores only a fingerprint. The store is a bounded schema-versioned map with at most 8 identities, one short store-wide `create_new` lease, atomic publication, legacy single-entry migration, 10-second abandoned-lease recovery, and no network await while the lease is held. OpenAI organization selection is not supported; the key's organization is the request scope.
