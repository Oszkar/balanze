...
- Claude JSONL: local file I/O - read **incrementally** via per-file byte cursor (`HashMap<PathBuf, FileCursor { byte_pos, mtime, size, identity }>`). Full reparse only on launch or `refresh_now()`. Detect atomic replacements by platform file identity (device/inode on Unix, volume/file-index on Windows) plus in-place rewrites and truncations by mtime/size, then replace that file's owned event contribution before cross-file deduplication. Decode only newline-terminated byte prefixes so an incomplete UTF-8 codepoint parks the cursor; skip invalid complete UTF-8 records and advance so corruption cannot stall later appends.
