...
- **Single writer.** `anthropic_oauth` is the only crate that reads or writes Claude Code's OAuth credential. Writes happen only to a credential file Balanze owns the refresh of: the refreshed-token write-back is atomic tmp + fsync + rename, perms-preserving, reuses Anthropic's file, touches only `claudeAiOauth` token fields, and never regresses a concurrently-newer on-disk token. The macOS login Keychain entry belongs to Claude Code and is **read-only** - Balanze uses the token while valid and never refreshes or writes a credential it does not own; an expired Keychain token surfaces `OAuthError::CredentialExpiredReadOnly` ("re-run `claude login`") rather than a refresh. The Keychain read shells out to `/usr/bin/security` (no new dep, keeps `keychain` the sole keyring-importing crate). We do not mirror, persist, or back up the credential's contents anywhere.
