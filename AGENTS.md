...
| Watcher cadence | `Settings::oauth_poll_interval_secs` (default 300; serde-default 300 on absent key). The Anthropic poller clamps to `max(300, value)`. The OpenAI watcher may wake on that cadence, but every Costs request is independently governed by the durable 300-second gate in `openai_client` |
