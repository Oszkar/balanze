...
9. **Frontend ↔ backend goes through the fixed IPC contract.** Commands: `get_snapshot`, `get_history`, `refresh_now`, `hide_window`, `resize_popover`, `set_api_key`, `validate_api_key`, `has_api_key`, `clear_api_key`, `get_settings`, `set_settings`, `get_statusline_status`, `set_statusline_wired`. Events: `usage_updated`, `degraded_state`. Adding to this surface needs a design-doc update first.
...
**Status:** implemented: `get_snapshot`, `refresh_now`, `hide_window`, `resize_popover`, `set_api_key`, `validate_api_key`, `has_api_key`, `clear_api_key`, `get_settings`, `set_settings`, `get_statusline_status`, `set_statusline_wired`, `usage_updated`, `degraded_state`. Everything except `get_snapshot` / `refresh_now` / `validate_api_key` is a synchronous command (Tauri runs them off the async runtime); `validate_api_key` is async because it makes a network request (it delegates to `watcher::validate_openai_key`). The key commands touch only the keychain (the user-supplied OpenAI key; Anthropic uses Claude Code's OAuth) and never return the value; `set_statusline_wired` delegates to `claude_statusline` and is no-clobber. `set_settings` / `set_api_key` / `clear_api_key` live-apply: they send `StateMsg::SettingsChanged` (coordinator clears disabled cells) and signal the host to re-spawn the watcher, so provider toggles take effect without a restart. Planned: `get_history` (the durable local history it reads is deferred - revisited post-1.0 with the dashboard; the v0.4.x CLI `export` re-derives the series from JSONL on each run and persists nothing, so `get_history` stays planned, tied to whenever storage lands). `refresh_now` today re-emits the current snapshot to the sink (repaint + catch-up); an on-demand provider re-poll is a later follow-up.
