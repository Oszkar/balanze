...
9. **Frontend ↔ backend goes through the fixed IPC contract.** Commands: `get_snapshot`, `get_history`, `refresh_now`, `set_api_key`, `get_settings`, `set_settings`, `get_statusline_status`, `set_statusline_wired`. Events: `usage_updated`, `degraded_state`. Adding to this surface needs a design-doc update first.
...
**Status:** implemented: `get_snapshot`, `refresh_now`, `set_api_key`, `get_settings`, `set_settings`, `get_statusline_status`, `set_statusline_wired`, `usage_updated`, `degraded_state`. `set_api_key` / `get_settings` / `set_settings` / `get_statusline_status` / `set_statusline_wired` are synchronous commands (Tauri runs them off the async runtime). `set_api_key` is wired to the keychain (only the user-supplied OpenAI key; Anthropic uses Claude Code's OAuth) and never returns or logs it; `set_statusline_wired` delegates to `claude_statusline` and is no-clobber (won't overwrite or strip another tool's `statusLine`). Planned: `get_history` (v0.3.3, with durable SQLite history + the sparkline). `refresh_now` today re-emits the current snapshot to the sink (repaint + catch-up); an on-demand provider re-poll is a v0.3.x follow-up.
