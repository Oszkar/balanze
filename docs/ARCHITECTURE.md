...
| Command | `replace_statusline` | Replace a foreign `statusLine.command` with Balanze's, backing the displaced command up to `settings.statusline.replaced_command` before writing. Rolls back the backup if the wire step fails. Only Claude Code's `settings.json` is touched - the foreign tool's own config files are never read or modified. |
