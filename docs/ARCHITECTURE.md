...
**Status:** implemented: `get_snapshot`, `refresh_now`, `set_api_key`, `get_settings`, `set_settings`, `usage_updated`, `degraded_state`. `set_api_key` / `get_settings` / `set_settings` are synchronous commands (Tauri runs them off the async runtime), wired to the keychain + `settings.json` behind the settings UI; `set_api_key` only accepts the user-supplied OpenAI key (Anthropic uses Claude Code's OAuth) and never returns or logs it. Planned: `get_history` (v0.3.3, with durable SQLite history + the sparkline). `refresh_now` today re-emits the current snapshot to the sink (repaint + catch-up); an on-demand provider re-poll is a v0.3.x follow-up.
