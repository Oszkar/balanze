...
8. **`src-tauri` and `balanze_cli` are glue, not logic.** Both compose backend crates into a `Snapshot`. The source-orchestration policy lives in `snapshot_composer::compose`; both entry-points run it. The JSONL → window+cost math is further shared via `state_coordinator::summarize_jsonl`, which `compose` (one-shot CLI) and the live coordinator merge (watcher path) both call — so the watcher cannot diverge on the window anchor. Identical inputs ⇒ identical `Snapshot` cells (parity-tested in `integration_4quadrant::compose_parity_against_fixtures`).
