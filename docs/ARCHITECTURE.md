...
9. **Frontend ↔ backend goes through the fixed IPC contract.** Commands: `get_snapshot`, `get_history`, `refresh_now`, `set_api_key`, `has_api_key`, `clear_api_key`, `get_settings`, `set_settings`, `get_statusline_status`, `set_statusline_wired`. Events: `usage_updated`, `degraded_state`. Adding to this surface needs a design-doc update first.
