...
- Tauri 2 popover/tray UI: color-shifting gauge tray icon, hidden-on-launch popover with one progress bar per Anthropic cadence + reset sublines, burn sparkline, the predictive countdown from v0.2, and the 4-quadrant tiles. The tiles render the **layered source/confidence model** Track C/D established — REAL billed (`extra_usage` overage) vs ESTIMATE (JSONL list-price, "leverage not spend") vs Claude's session-cost estimate vs server quota % — each visually tagged so no two are conflatable; primary quota source is the statusline feed, with OAuth shown as the stale/fallback state. `tauri-plugin-single-instance` wired so the user cannot double-launch.
- Settings UI: paste API keys, save to OS keychain — and with a real key-input box the keychain code is exercised on both platforms, so this is where the **`keyring` → `keyring-core` (v4) migration** lands (fixes the v0.1 Windows keychain no-op). Also surfaces the Track D `statusLine` wiring (the CLI `setup` does it headless; the settings UI shows/edits it).
...
- Anthropic Console cookie-paste integration — **demoted (2026-05-19): not a committed v0.3 item.** Its only unique signal is the *current* prepaid balance; `extra_usage` (Track C) already gives the real overage spent/limit and statusline (Track D) gives authoritative quota — so the fragile/unofficial scrape (§3.3) is implemented only if a concrete user need surfaces. If built: cookie-paste UX, tile shows `auth_expired` and prompts a re-paste on 401.
