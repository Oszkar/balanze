...
- A per-turn cache so the line is cheap and stays polite (AGENTS.md 3.1) even though it runs on every conversation turn.
...
| D2 | Cross-provider data sourcing | Hybrid | Read the running watcher/desktop snapshot when fresh; self-compose with a per-turn cache otherwise. Works whether or not the desktop app runs, and never violates 3.1. |
...
Owns the segment model, the renderer (snapshot + config -> string), the per-turn cache, and the cross-provider self-compose path. `balanze-cli statusline` and a future desktop statusline surface stay thin over it.
...
## 5. Hybrid sourcing and the per-turn cache
...
- OpenAI billed $: `openai_client` calls the Admin Costs API through the per-turn cache.

### 5.3 The cache (PR3)

A transcript-path-keyed file cache under `<ProjectDirs.cache>/statusline/` (NOT written next to the user's transcript - PoLP):

- OpenAI costs: mtime-TTL 300s (this IS the 3.1 5-minute politeness gate), stale-while-updating so the line never blanks, a negative-failure cooldown so a failing API is not hammered, and an OpenAI-key fingerprint that invalidates the entry on key rotation.
...
3. PR3 - Self-compose fallback + per-turn cache. When no fresh snapshot, compose Codex (local) + OpenAI (cached API) directly (not via the OAuth-touching composer); the transcript-keyed mtime-TTL cache with stale-while-updating, negative cooldown (= the 3.1 gate), and key fingerprint. The only PR that adds statusline-initiated network, landing with its politeness gate.
