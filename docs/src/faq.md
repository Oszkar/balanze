...
At UTC month rollover, an attempt started just before the new month can keep validation and full OpenAI data deferred for up to 5 minutes. This is the deliberate machine-wide request guarantee, not evidence that the replacement key is invalid. Retry after the reservation expires.
