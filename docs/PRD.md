...
**Current status: shipped through v0.4.1 (CLI maturity).** The data layer, the live spine, the Tauri popover + settings, the UI polish pass, and the first-class CLI surface (status/watch/doctor/export/completions, the exit-code taxonomy) are done, and the product is useful day to day. The remaining v0.4.x work (v0.4.2) productizes the statusline; v0.5 ships it to other people; v0.6 adds alerts; v0.7 deepens settings and theming.
