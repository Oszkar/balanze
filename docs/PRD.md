...
**v0.5.2 - Legibility.** The **"how it works" writeup** - the data-provenance model (the measured-only matrix + the leverage insight), the "measured status, not forecasts" call, and the actor-model architecture / the twelve boundaries - and the **mdBook docs site**: the skeleton, a GitHub Pages deploy, and a rendered home for the user guide and the writeup. The surface-consistency design record stays linked from the guide rather than promoted to its own page - it reads as an internal review artifact now that the writeup documents the actual decision. The generator decision was recorded in v0.5.0.
