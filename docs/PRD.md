...
**Current status: shipped through v0.4.0; v0.4.1 (CLI maturity) implemented on `main`, release pending.** The data layer, the live spine, the Tauri popover + settings, and the UI polish pass are done, and the product is useful day to day. The v0.4.1 CLI-maturity work (status/watch/doctor/export/completions, the exit-code taxonomy) has landed on `main` and is in dogfooding before tagging. The remaining v0.4.x work (v0.4.2) productizes the statusline; v0.5 ships it to other people; v0.6 adds alerts; v0.7 deepens settings and theming.
