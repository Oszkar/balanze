...
**v0.5.3 - Support & hardening.** A **price-table refresh script** (`scripts/refresh-claude-prices.*`) mechanizing the vendored LiteLLM Anthropic price-table refresh, a **"Send Logs"** menu item bundling rotated logs + a recent state snapshot for support, and the accumulated correctness debt: one Codex window-selection contract; cross-process settings transactions that lock before reloading and apply field-level intent, including recoverable ordering for keychain and statusline multi-resource changes; a provider-owned, provenance-safe OpenAI Costs gate shared by every caller; and the deferred pace-display call.
