...
**v0.5.2 - Legibility.** The **mdBook docs site**: the skeleton, a GitHub Pages deploy, and a rendered home for the user guide, which already carried the data-provenance model (the measured-only matrix + the leverage insight). A standalone "how it works" writeup was drafted and then folded back in rather than kept as its own chapter - the data-provenance section duplicated the guide almost verbatim, the "measured status, not forecasts" call (why the EWMA predictor was retired in favor of honest pace facts) is now a one-line aside in the guide's pace section, and the actor-model architecture / the twelve boundaries stay in `ARCHITECTURE.md` rather than getting a dedicated site page - the same call made for the surface-consistency design record: internal engineering detail, not user-facing. The generator decision was recorded in v0.5.0.
