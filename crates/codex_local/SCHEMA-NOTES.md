...
- **Per-model limits share the file (`limit_id`).** A session that invokes Spark appends `rate_limits` with `limit_id: "codex_bengalfox"` / `limit_name: "GPT-5.3-Codex-Spark"`, carrying THAT MODEL's window (typically 0%), interleaved with - and often trailing - the account-wide `limit_id: "codex"` blocks. The parser previously took the last `token_count` in the newest file regardless, so a Spark-trailing session would report ~0% as the account quota: a silent under-report. It now skips any block whose `limit_id` is a string other than `"codex"` (missing/null `limit_id` still counts as the account limit, for pre-`limit_id` payloads). A session with only per-model blocks yields `Ok(None)`, not `SchemaDrift`, so the walker falls through to the next-newest session.
