<script lang="ts">
  ...
  // before any child mounts. This component is only ever mounted in dev contexts
  // (the route gates it; the standalone entry is non-prod), so the only guard
  // needed is browser-only (ssr=false), kept for safety.
